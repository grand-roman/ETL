...
def query_part_for_where(data: Union[list, tuple]) -> str:
    ...
    return f"IN {tuple(data)}" if len(data) > 1 else f"= '{data[0]}'"
...
def query_film_works_by_genres(genre_ids: list) -> str:
    ...
    return f"""
        ...
        WHERE gfw.genre_id {query_part_for_where(data=genre_ids)}
...
def query_film_works_by_persons(person_ids: list) -> str:
    ...
    return f"""
        ...
        WHERE pfw.person_id {query_part_for_where(data=person_ids)}
        ORDER BY fw.updated_at;
    """
...
def query_film_works_by_ids(film_work_ids: tuple) -> str:
    ...
    return f"""
        ...
        WHERE fw.id {query_part_for_where(data=film_work_ids)};
    """
